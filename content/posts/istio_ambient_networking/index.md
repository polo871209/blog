---
title: "Istio Ambient Mesh Networking: A Deep Dive Into ztunnel, HBONE, and Waypoints"
date: 2026-04-21
lastmod: 2026-04-21
description: "An in-depth walkthrough of how Istio ambient mode routes traffic without sidecars — covering ztunnel, HBONE tunneling, waypoint proxies, and in-pod traffic redirection"
...
tags: ["istio", "kubernetes", "service-mesh", "networking", "ambient", "ztunnel", "hbone"]
draft: true
---
...
The `/var/run/ztunnel/ztunnel.sock` Unix domain socket is the only channel between them. The istio-cni agent passes the **pod's netns file descriptor** over this socket. Ztunnel then binds listeners *inside* the pod's namespace without ever needing namespace-enumeration privileges itself.
...
- **Passable over Unix domain sockets** — the `SCM_RIGHTS` control message lets one process hand an open fd to another process. The receiver gets a *new* fd number pointing to the *same* kernel object. Works across processes that share nothing else.
...
- [Ztunnel Architecture (GitHub)](https://github.com/istio/ztunnel/blob/master/ARCHITECTURE.md)
...
- [HTTP CONNECT Method (MDN)](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/CONNECT)
